...
const DEFAULT_BG = '#faf6ef'; // matches the app's default cream background
const DEFAULT_AVATAR = { type: 'text', value: '(ー_ー)' };
const AVATAR_TEXT_OPTIONS = ['(ー_ー)', '(^_^)', '(T_T)', '(・_・)', '(¬_¬)', '(o_o)', '(>_<)', '(*_*)', '(-_-)zzz', 'ヽ(・∀・)ﾉ'];
const MAX_AVATAR_IMAGE_BYTES = 300 * 1024; // small, since this gets stored permanently on the account
...
io.on('connection', socket => {
  ...
  socket.on('claim-or-login', ({ name }) => {
    ...
    if (!existing) {
      ...
      socket.emit('login-success', {
        ...
        backgroundColor: accounts[key].backgroundColor,
  ...
  socket.on('login-with-password', ({ name, password }) => {
    ...
    socket.emit('login-success', {
      name: account.name,
      history: history.slice(-MAX_HISTORY),
      backgroundColor: account.backgroundColor || DEFAULT_BG,
  ...
  socket.on('set-background-color', ({ color }) => {
    if (!authedName) return;
    const clean = String(color || '');
    if (!HEX_COLOR_RE.test(clean)) {
      socket.emit('profile-error', { field: 'color', message: 'That color is not valid.' });
      return;
    }
    const key = authedName.toLowerCase();
    const account = accounts[key];
    if (!account) return;
    account.backgroundColor = clean;
    saveAccounts();
    socket.emit('profile-updated', { backgroundColor: clean });
  });

  socket.on('set-avatar', ({ type, value, dataUrl, mime }) => {
    if (!authedName) return;
    const key = authedName.toLowerCase();
    const account = accounts[key];
    if (!account) return;
    ...
    if (type === 'text') {
      ...
      if (!clean) {
        socket.emit('profile-error', { field: 'avatar', message: 'Pick or type something for your avatar.' });
        return;
      }
      account.avatar = { type: 'text', value: clean };
    } else if (type === 'image') {
      if (!dataUrl || typeof dataUrl !== 'string' || dataUrl.length > MAX_AVATAR_IMAGE_BYTES * 1.4) {
        socket.emit('profile-error', { field: 'avatar', message: 'That image is too big. Try something under 300KB.' });
        return;
      }
