...
io.on('connection', socket => {
  ...
  // Self-service recovery from the login gate: type a name, get a fresh
  // password, and get logged straight in with it. No other verification —
  // see the caveat about this in README.md before relying on it heavily.
  socket.on('recover-password', ({ name }) => {
    const key = String(name || '').trim().toLowerCase();
    const account = accounts[key];
    if (!account) {
    ...
    socket.data.name = account.name;
    activeSockets.set(account.name, socket);
    socket.emit('password-recovered', { name: account.name, password: newPassword });
    socket.emit('login-success', {
      name: account.name,
      history: history.slice(-MAX_HISTORY),
      avatar: account.avatar || DEFAULT_AVATAR
  ...
  socket.on('change-my-password', () => {
    ...
    account.hash = hashPassword(newPassword, salt);
    saveAccounts();
    socket.emit('password-changed', { password: newPassword });
  });
